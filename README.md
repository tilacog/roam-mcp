...
- **Write tools:** `create_node`, `update_node`, `delete_node`, `rename_node`, `append_to_node`, `prepend_to_node`, `add_link`, `insert_anchor`, `daily_capture`. Writes never touch `org-roam.db` directly. In `--read-only` mode they are removed from the tool router entirely — not listed, not callable.
  - `update_node` is the idempotent counterpart to `create_node`: keyed on `:ID:`, it edits a file-level node's title, body, tags, aliases, refs, or drawer properties in place without changing the ID (so backlinks survive). Pass `preview: true` for a dry run that returns the would-be file text without writing.
